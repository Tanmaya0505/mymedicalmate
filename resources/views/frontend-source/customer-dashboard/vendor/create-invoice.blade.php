...
<div class="content">
    ...
    <div class="t-row">
      ...
      <div class="cell text-center" data-title="Qty">
        <form novalidate id="add-medicine-form-{{$med->id}}" action="{{url('/vendor/update-medicine/'.$med->id)}}" method="post">
                  @csrf
        <input type="text" name="quantity" class="quantitylist" value="{{$med->quantity}}">
      </form>
      </div>
    ...
    @empty
    @endforelse
    <div class="t-row" style="background-color:#fff">
      <div class="cell" data-title="#">
      ...
      <div class="cell" data-title="Amount">
        <button type="button" class="btn btn-danger btn-fill btn-sm float-right" data-toggle="modal" data-target="#addMedicine">+</button>
      </div>
    </div>
                                ...
                                <div class="invoice-table">
                                    ...
                                    <div class="row contact-inf mb50">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <p>{{@$customer_address['patient_name']}}</p>
                                            <p>Call:{{@$customer_address['patient_mobile']}}</p>
                                            <p>{{@$customer_address['patient_email']}}</p>
                                            <p>From: {{@$customer_address['patient_from']}}</p>
                                        </div>
                                        <div class="col-md-6">
                                            <p>Invoice created by::</p>
                                            <p>{{$vendorDetails->name_of_store}} </p>
                                            
                                        </div>
                                        
                                    </div>
                                    @if(count($allmedicines) > 0 && $allmedicines[0]->created_by==1)
                                        <div class="row contact-inf mb50">
                                            <a href="{{url('vendor/send-invoice/3/'.$prescription->order_id)}}" class="btn btn-info btn-sm col-6" >Send to Patient & Med mate</a>
                ...
                <form novalidate id="add-medicine-form" action="{{url('/vendor/add-medicine')}}" method="post">
                  @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" name="quantity" onkeypress="return isNumber(event)" required="" id="quantity">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" name="price" onkeypress="return isNumber(event)" required="" id="price">
